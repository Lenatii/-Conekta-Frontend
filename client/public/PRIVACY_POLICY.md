# CONEKTA PRIVACY POLICY

**Last Updated:** November 23, 2025  
**Effective Date:** November 23, 2025

---

## 1. INTRODUCTION

Welcome to **CONEKTA** ("we," "our," or "us"). We are committed to protecting your personal information and your right to privacy. This Privacy Policy explains how we collect, use, disclose, and safeguard your information when you use our services through WhatsApp (Mama Dennis chatbot) and our platform.

**CONEKTA** is a marketplace platform connecting tenants with landlords, and clients with service providers (fundis) in Nakuru, Kenya. We also provide identity verification services through our UBARU system.

By using CONEKTA, you agree to the collection and use of information in accordance with this Privacy Policy and the **Kenya Data Protection Act, 2019**.

---

## 2. INFORMATION WE COLLECT

### 2.1 Personal Information You Provide

When you register or use CONEKTA services, we collect:

**For All Users:**
- Full name
- Phone number (WhatsApp number)
- City and area (location)
- Profile information

**For Landlords:**
- Property details (location, rent, amenities)
- Property photos
- Next of kin information (name, phone, relationship)
- Emergency contact information

**For Fundis (Service Providers):**
- Service category (plumber, electrician, etc.)
- Tools owned
- Experience and skills
- Service area
- ID verification information (through UBARU)

**For Identity Verification (UBARU):**
- National ID number
- ID card photo
- Selfie photo (for verification)

### 2.2 Information Collected Automatically

- WhatsApp messages and conversation history with Mama Dennis
- Usage data (features used, search queries)
- Device information (phone model, operating system)
- Timestamps of interactions

### 2.3 Payment Information

- M-Pesa phone number (for contact reveal payments)
- Transaction records
- Payment timestamps

**Note:** We do NOT store M-Pesa PIN or other sensitive payment credentials.

---

## 3. HOW WE USE YOUR INFORMATION

We use your information to:

### 3.1 Provide Services
- Connect tenants with landlords
- Connect clients with service providers (fundis)
- Process contact reveal requests
- Verify identities through UBARU
- Send notifications via WhatsApp

### 3.2 Improve Services
- Analyze usage patterns
- Improve Mama Dennis AI responses
- Enhance matching algorithms
- Develop new features

### 3.3 Security & Fraud Prevention
- Verify user identities
- Prevent fake listings
- Detect and prevent fraud
- Protect against scams

### 3.4 Legal Compliance
- Comply with Kenya Data Protection Act, 2019
- Respond to legal requests
- Enforce our Terms of Service
- Protect our rights and property

---

## 4. HOW WE PROTECT YOUR INFORMATION

We take data security seriously and implement multiple layers of protection:

### 4.1 Encryption
- **All sensitive data is encrypted** using AES-256 encryption
- Phone numbers are encrypted in our database
- ID numbers are encrypted
- Next of kin information is encrypted
- M-Pesa phone numbers are encrypted

### 4.2 Access Controls
- Limited access to personal data (need-to-know basis)
- Admin authentication required
- Role-based access control

### 4.3 Technical Security
- HTTPS/TLS encryption for data in transit
- Secure API endpoints
- Rate limiting to prevent attacks
- Regular security audits
- Secure file storage for photos

### 4.4 Physical Security
- Database hosted on secure servers
- Regular backups (encrypted)
- Disaster recovery procedures

---

## 5. HOW WE SHARE YOUR INFORMATION

### 5.1 With Other Users (Contact Reveals)

When someone pays KES 100 to reveal your contact:
- **Landlords:** Your phone number is shared with the tenant
- **Fundis:** Your phone number is shared with the client

**You control what you list!** Only information you provide is shared.

### 5.2 With Service Providers

We share limited data with:
- **Twilio** (WhatsApp messaging)
- **Instasend** (M-Pesa payments)
- **Render** (hosting provider)

These providers are contractually obligated to protect your data.

### 5.3 With Law Enforcement

We may disclose information if required by law or to:
- Comply with legal processes
- Protect our rights
- Prevent fraud or illegal activity
- Protect user safety

### 5.4 We DO NOT:
- ❌ Sell your data to third parties
- ❌ Share your data for marketing purposes
- ❌ Give away your contact without payment
- ❌ Share your ID photos publicly

---

## 6. YOUR RIGHTS (Kenya Data Protection Act, 2019)

You have the right to:

### 6.1 Access Your Data
Request a copy of all personal data we hold about you.

### 6.2 Correct Your Data
Update or correct inaccurate information.

### 6.3 Delete Your Data
Request deletion of your account and data (subject to legal requirements).

### 6.4 Object to Processing
Object to how we use your data.

### 6.5 Data Portability
Receive your data in a portable format.

### 6.6 Withdraw Consent
Withdraw consent for data processing at any time.

**To exercise your rights, contact:**
- **Data Protection Officer:** dpo@conekta.co.ke
- **WhatsApp:** +254 797 446155
- **Message Mama Dennis:** Type "Privacy Rights"

---

## 7. DATA RETENTION

We retain your data for as long as:
- Your account is active
- Needed to provide services
- Required by law (minimum 7 years for financial records)

After account deletion:
- Personal data is permanently deleted within 30 days
- Transaction records retained for 7 years (legal requirement)
- Anonymized data may be retained for analytics

---

## 8. CHILDREN'S PRIVACY

CONEKTA is **NOT intended for users under 18 years old**. We do not knowingly collect data from children. If we discover we have collected data from a child, we will delete it immediately.

---

## 9. INTERNATIONAL DATA TRANSFERS

Your data is primarily stored and processed in Kenya. If we transfer data internationally, we ensure adequate protection through:
- Standard contractual clauses
- Adequacy decisions
- Appropriate safeguards

---

## 10. COOKIES AND TRACKING

We do **NOT use cookies** as CONEKTA operates primarily through WhatsApp. However, if you visit our website, we may use:
- Essential cookies (for functionality)
- Analytics cookies (to improve service)

You can disable cookies in your browser settings.

---

## 11. THIRD-PARTY LINKS

Our service may contain links to third-party websites. We are **NOT responsible** for their privacy practices. Please review their privacy policies.

---

## 12. CHANGES TO THIS PRIVACY POLICY

We may update this Privacy Policy from time to time. We will notify you of changes by:
- Posting the new policy on our platform
- Sending a WhatsApp message via Mama Dennis
- Updating the "Last Updated" date

Continued use after changes constitutes acceptance.

---

## 13. CONTACT US

If you have questions about this Privacy Policy or your data:

**Data Protection Officer:**
- **Email:** dpo@conekta.co.ke
- **Phone:** +254 797 446155
- **WhatsApp:** Message Mama Dennis
- **Address:** Nakuru, Kenya

**Office of the Data Protection Commissioner (Kenya):**
- **Website:** www.odpc.go.ke
- **Email:** info@odpc.go.ke
- **Phone:** +254 20 2664000

---

## 14. LEGAL BASIS FOR PROCESSING (Kenya Data Protection Act, 2019)

We process your data based on:

1. **Consent:** You provide explicit consent when registering
2. **Contract:** Processing necessary to provide services
3. **Legal Obligation:** Compliance with Kenyan laws
4. **Legitimate Interest:** Fraud prevention, security, service improvement

---

## 15. DATA BREACH NOTIFICATION

In the event of a data breach affecting your personal information, we will:
- Notify you within **72 hours**
- Inform the Office of the Data Protection Commissioner
- Provide details of the breach and remedial actions
- Offer support and guidance

---

## 16. AUTOMATED DECISION-MAKING

We use AI (Mama Dennis) to:
- Match tenants with properties
- Match clients with fundis
- Detect potential fraud

You have the right to:
- Request human review of automated decisions
- Challenge AI-based decisions
- Understand how decisions are made

---

## 17. YOUR CONSENT

By using CONEKTA, you consent to:
- Collection of your personal information as described
- Processing of your data for the purposes stated
- Sharing of your contact information when someone pays to reveal it
- Use of AI for matching and fraud detection

**You can withdraw consent at any time** by deleting your account.

---

## 18. COMPLAINTS

If you believe we have violated your privacy rights, you can:

1. **Contact us first:** dpo@conekta.co.ke
2. **File a complaint** with the Office of the Data Protection Commissioner:
   - Website: www.odpc.go.ke
   - Email: complaints@odpc.go.ke
   - Phone: +254 20 2664000

---

**Thank you for trusting CONEKTA with your information!**

We are committed to protecting your privacy and building a safe, trustworthy marketplace for Nakuru.

---

**CONEKTA**  
*Connecting People, Building Trust*

📱 WhatsApp: +254 797 446155  
🌐 Nakuru, Kenya  
📧 dpo@conekta.co.ke

---

*This Privacy Policy is governed by the laws of Kenya.*
